...
<head>
    ...
    <link rel="stylesheet" href="resources/css/board/qbdetail.css">
    <meta charset="UTF-8">
    <title>디테일</title>
    <style>
    ...
    <div>
        <header>
         ...
         <main>
            ...
            <div id="directionDiv">
                <span>댓글</span>
                <button onclick="location.href='qblist'">목록</button>
                <button onclick="location.href='qbdetail?seq=${Detail.seq}&code=U'">수정</button>
                <button onclick="location.href='qbdelete?seq=${Detail.seq}'">삭제</button>
            </div>
            <div id="commentDiv">
            <c:forEach items="${cment}" var="cment">
                <table> 
                    <tr>  
                        <td>${cment.studentId }<span>${cment.regDate}</span>
                        <a href="cqbdelete?seq=${cment.seq}&root=${cment.root}">x</a></td>
                    </tr>
                    <tr>
                        <td>${cment.contents}</td>
                    </tr>
                </table>
                </c:forEach>
            </div>
            <div id="commentInsertDiv">
                <form action="cqbinsert">
                	<input type="hidden" value="${Detail.seq}" name="seq"> 
                    <input type="text" placeholder="아이디" name="studentId"><br>
                    <input type="text" placeholder="매너가 사람을 만든다"name="contents">
